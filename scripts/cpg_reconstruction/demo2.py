...
from demo_helper import process_ae
...
app.layout = dash.html.Div(children=[
    dash.dcc.Textarea(id='input-text1',style={'width': '50%', 'height': 300, 'display': 'inline-block'}),
    dash.dcc.Textarea(id='input-text2',style={'width': '50%', 'height': 300, 'display': 'inline-block'}),
...
@app.callback(
    ...
    [dash.Input("start_work", "n_clicks"), dash.Input("input-text1", "value"), dash.Input("input-text2", "value")]
    )
def start_bar(n, text1, text2):
    if n==0 or progress_memory < 1 or len(text1) < 2 or len(text2) < 2:
        return(0,)
    threading.Thread(target=start_work, args=(progress_queue,text1,text2)).start()
    return(0,)


def start_work(output_queue, text1, text2):
    process_ae(text1, text2, output_queue)
    return(None)
