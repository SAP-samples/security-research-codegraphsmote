...
from experiments.datasets.smote import SMOTEDataset
...
from utils import get_dataset, get_classification_model, get_ae_model
...
assert CLASSIFICATION_DATASET["type"] == "SMOTE", "This script is only valid for SMOTE-Datasets"
...
CLASSIFICATION_DATASET["model"]["encoder"]["features"] = full_dataset.get_input_size()
CLASSIFICATION_DATASET["model"]["encoder"]["edge_dim"] = full_dataset.get_edge_size()
ae_model = get_ae_model(CLASSIFICATION_DATASET["model"]["encoder"], CLASSIFICATION_DATASET["model"]["decoder"])
ae_model.load(CLASSIFICATION_DATASET["model"]["checkpoint"])
...
for percentage in PERCENTAGES:
    ...
    with Logger("results/" + EXPERIMENT_NAME + "/out.log"):
        for i in range(RUNS):
            ...
            dataset = SMOTEDataset(ae_model, reduced_dataset, CLASSIFICATION_DATASET)
